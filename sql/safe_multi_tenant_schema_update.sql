-- Safe Multi-Tenant Schema Update for School Isolation
-- This version handles missing school references and data integrity issues
-- Run this SQL in your Supabase SQL Editor

-- ========================================
-- 1. FIRST, CHECK AND FIX DATA INTEGRITY
-- ========================================

-- Check for courses with invalid school_id references
SELECT 'Checking for courses with invalid school_id...' as info;
SELECT 
  c.id,
  c.title,
  c.school_id,
  CASE 
    WHEN s.id IS NULL THEN 'MISSING SCHOOL'
    ELSE 'OK'
  END as status
FROM courses c
...
-- Get the default school ID for reference
DO $$
DECLARE
...
BEGIN
  ...
  -- Update courses with invalid school_id to use default school
  UPDATE courses 
  SET school_id = default_school_id
  WHERE school_id IS NOT NULL 
  AND school_id NOT IN (SELECT id FROM schools);
  
  -- Update courses with NULL school_id to use default school
  UPDATE courses 
  SET school_id = default_school_id
  WHERE school_id IS NULL;
  
  RAISE NOTICE 'Updated courses with invalid or NULL school_id to use default school: %', default_school_id;
END $$;
...
BEGIN
  ...
  RAISE NOTICE 'Updated profiles with NULL school_id to use default school: %', default_school_id;
END $$;

-- ========================================
-- 2. ENSURE SCHOOL_ID COLUMNS EXIST
-- ========================================
...
-- ========================================
-- 4. UPDATE RLS POLICIES FOR MULTI-TENANT
-- ========================================
...
-- ========================================
-- 5. CREATE SCHOOL-ISOLATED RLS POLICIES
-- ========================================
...
-- ========================================
-- 6. CREATE SAFE SCHOOL DATA MIGRATION FUNCTIONS
-- ========================================

-- Safe function to populate school_id for existing records
CREATE OR REPLACE FUNCTION safe_populate_school_ids()
RETURNS TABLE(
  table_name TEXT,
...
BEGIN
  ...
  -- Update exams with school_id from courses (with validation)
  UPDATE exams 
  SET school_id = COALESCE(c.school_id, default_school_id)
  FROM courses c
  WHERE exams.course_id = c.id 
  AND exams.school_id IS NULL
  AND EXISTS (SELECT 1 FROM schools WHERE id = COALESCE(c.school_id, default_school_id));
  
  RETURN QUERY SELECT 'exams', COUNT(*)::INTEGER, 0::INTEGER FROM exams WHERE school_id IS NOT NULL;

  -- Update exam_questions with school_id from exams (with validation)
  UPDATE exam_questions 
  SET school_id = COALESCE(e.school_id, default_school_id)
  FROM exams e
  WHERE exam_questions.exam_id = e.id 
  AND exam_questions.school_id IS NULL
  AND EXISTS (SELECT 1 FROM schools WHERE id = COALESCE(e.school_id, default_school_id));
  
  RETURN QUERY SELECT 'exam_questions', COUNT(*)::INTEGER, 0::INTEGER FROM exam_questions WHERE school_id IS NOT NULL;

  -- Update exam_sessions with school_id from exams (with validation)
  ...
  RETURN QUERY SELECT 'exam_sessions', COUNT(*)::INTEGER, 0::INTEGER FROM exam_sessions WHERE school_id IS NOT NULL;

  -- Update exam_answers with school_id from exam_sessions (with validation)
  UPDATE exam_answers 
  SET school_id = COALESCE(es.school_id, default_school_id)
  FROM exam_sessions es
  WHERE exam_answers.exam_session_id = es.id 
  AND exam_answers.school_id IS NULL
  AND EXISTS (SELECT 1 FROM schools WHERE id = COALESCE(es.school_id, default_school_id));
  
  RETURN QUERY SELECT 'exam_answers', COUNT(*)::INTEGER, 0::INTEGER FROM exam_answers WHERE school_id IS NOT NULL;

  -- Update assignments with school_id from courses (with validation)
  UPDATE assignments 
  SET school_id = COALESCE(c.school_id, default_school_id)
  FROM courses c
  ...
  RETURN QUERY SELECT 'assignments', COUNT(*)::INTEGER, 0::INTEGER FROM assignments WHERE school_id IS NOT NULL;

  -- Update assignment_submissions with school_id from assignments (with validation)
  UPDATE assignment_submissions 
  SET school_id = COALESCE(a.school_id, default_school_id)
  FROM assignments a
  WHERE assignment_submissions.assignment_id = a.id 
  AND assignment_submissions.school_id IS NULL
  AND EXISTS (SELECT 1 FROM schools WHERE id = COALESCE(a.school_id, default_school_id));
  
  RETURN QUERY SELECT 'assignment_submissions', COUNT(*)::INTEGER, 0::INTEGER FROM assignment_submissions WHERE school_id IS NOT NULL;
END;
$$ LANGUAGE plpgsql;

-- ========================================
-- 7. CREATE SCHOOL ISOLATION VALIDATION
-- ========================================
...
-- ========================================
-- 8. CREATE SCHOOL FILE CLEANUP FUNCTION
-- ========================================
...
-- ========================================
-- 9. RUN SAFE MIGRATION AND VALIDATION
-- ========================================

-- Populate school IDs for existing records safely
SELECT 'Running safe migration...' as info;
SELECT * FROM safe_populate_school_ids();

-- Validate school isolation
SELECT 'Validating school isolation...' as info;
...
SELECT '✅ Safe multi-tenant migration completed successfully!' as status;
SELECT 'All school_id columns added with proper foreign key constraints.' as details;
SELECT 'RLS policies created for complete school isolation.' as security;
SELECT 'File cleanup functions available for school-specific operations.' as cleanup;
