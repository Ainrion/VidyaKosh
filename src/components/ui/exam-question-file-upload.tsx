...
interface ExamQuestionFileUploadProps {
...
export function ExamQuestionFileUpload({
  onFileSelect,
  onFileRemove,
  onFileUpload,
  examId,
  questionId,
  acceptedTypes = ['.pdf', '.doc', '.docx', '.txt', '.xml', '.jpg', '.jpeg', '.png'],
  maxSizeMB = 50,
  disabled = false,
  className,
  existingFile,
  placeholder = "Drag and drop your question file here, or click to browse"
}: ExamQuestionFileUploadProps) {
  ...
  const uploadFileToServer = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/exams/upload-question-file', {
        method: 'POST',
        body: formData
      })
  ...
  }, [validateFile, onFileSelect, onFileRemove, onFileUpload, examId, questionId])
        ...
        <div className="p-6 text-center">
          ...
          {isUploading ? (
            ...
            <div className="space-y-4">
              <Upload className={cn(
                "h-12 w-12 mx-auto",
                error ? "text-red-500" : disabled ? "text-gray-300" : "text-gray-400"
              )} />
              <div>
