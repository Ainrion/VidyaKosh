...
export async function POST(request: NextRequest) {
  try {
    // Use service role client to completely bypass RLS
    ...
    if (existingProfile) {
      // If profile exists, return it as success instead of error
      return NextResponse.json({ 
        success: true, 
        profile: existingProfile,
        message: 'Profile already exists and is ready to use' 
      })
    }

    // Create the profile using service role (completely bypasses RLS)
    ...
    if (createError) {
      console.error('Simple profile creation error:', createError)
      return NextResponse.json({ 
    ...
    return NextResponse.json({ 
      success: true, 
      profile: profile,
      message: 'Profile created successfully using simple bypass' 
    })
  } catch (error) {
